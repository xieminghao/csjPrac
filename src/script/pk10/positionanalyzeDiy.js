	            ...
	            var viewmodel = avalon.define('analyze', function (vm) {
	                ...
	                vm.calculate = function (a) {
	                    ...
	                    for (var j = viewmodel.$cars.length - 1; j > 0; j--) {
	                        d.min = Math.min(d.min, d.nums[viewmodel.$cars[j]-1]);
	                        d.max = Math.max(d.max, d.nums[viewmodel.$cars[j]-1]);
	                    }
	                    d.kd = d.max - d.min;
	                    return d;
	                }
	                ...
	                vm.$watch("flagPeriod", function (value, oldvalue) {
	                    if (value == -1) return;
	                    if (viewmodel.analyDatas.length <= 0||viewmodel.$cars.length<3) return;
