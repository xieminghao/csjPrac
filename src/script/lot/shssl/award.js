(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('/shssl/' + _page, { t: Math.random() }, function (text) {
    ...
    function loadAwardTimes() {
        $.get('/shssl/ajax', { ajaxhandler: 'GetShsslAwardTimes', t: Math.random() }, function (data) {
            //请求到数据后需要做的事情
            ...
            var leavePeriod = 23 - cpNumber;
            if (leavePeriod == 0) {
                var d = new Date();
                var nd = new Date(data.next.awardTime.split(' ')[0].replace("-", "/", "gi"));
                if (d.getDate() == nd.getDate()) leavePeriod = 23;
            }
...
function updateHistoryRecord() {
    $.get('/shssl/ajax', { ajaxhandler: 'GetNewestRecord', t: Math.random() }, function (data) {
