(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('/fc3d/' + _page, { t: Math.random() }, function (text) {
    ...
    var awardTick = function () {
        $.get('/fc3d/ajax', { ajaxhandler: 'GetAwardData', t: Math.random() }, function (data) {
            requireCount += 1;
            if ((data.current.periodNumber != currentPeriodNumber) && currentPeriodNumber != -1) {
                timeInterval = 300000;
                window.setTimeout(afterAwarded, 600000);
            ...
            var _time = parseInt(parseInt(data.next.awardTimeInterval) + timeInterval + parseInt(Math.random() * 300000));
            window.setTimeout(awardTick, data.next.awardTimeInterval < 10 ? 300000 : _time);
            timeInterval = 0;
        }, 'json').error(function () {
            if (errorCount < 20) {
                window.setTimeout(awardTick, 300000 + Math.random() * 10000);
                errorCount++;
            }
        });
    ...
    function loadAwardTimes() {
        $.get('/fc3d/ajax', { ajaxhandler: 'GetAwardTimes', t: Math.random() }, function (data) {
            //请求到数据后需要做的事情
            ...
            if (data.current.periodNumber != cpNumber) {
                ...
                countDownTimer = window.setInterval(function () {
                    ...
                    showCountDown(cpNextAwardTimeInterval, data.next.periodNumber, "fc3d")
                }, 1000);
            }
            ...
            loadAwardTimesTimer = window.setTimeout(loadAwardTimes, data.next.awardTimeInterval < 10 ? 300000 : data.next.awardTimeInterval + 1000);
        }, 'json').error(function () {
            if (errorCount < 20) {
                window.setTimeout(loadAwardTimes, 300000 + Math.random() * 10000);
                errorCount++;
            }
        });
        if (errorCount >= 5) {
...
function updateHistoryRecord() {
    $.get('/fc3d/ajax', { ajaxhandler: 'GetNewestRecord', t: Math.random() }, function (data) {
