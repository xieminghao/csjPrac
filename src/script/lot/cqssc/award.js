(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('/shishicai/' + _page, { t: Math.random() }, function (text) {
    ...
    function loadAwardTimes() {
        $.get('/shishicai/ajax', { ajaxhandler: 'GetCqsscAwardTimes', t: Math.random() }, function (data) {
            //请求到数据后需要做的事情
...
function updateHistoryRecord() {
    $.get('/shishicai/ajax', { ajaxhandler: 'GetNewestRecord', t: Math.random() }, function (data) {
...
function LoadCqssscTipSet() {
    ...
    var tip_num = parseInt($.cookie("cqssc_tip_num") == null ? 1 : $.cookie("cqssc_tip_num"));
    if (1 == tip_numstat) {
