(function () {
    ...
    var awardTick = function () {
        $.get('/xyft/ajax', { ajaxhandler: 'GetXyftAwardData', t: Math.random() }, function (data) {
            ...
            if ((data.current.periodNumber != currentPeriodNumber) && currentPeriodNumber != -1) {
                timeInterval = 38000;
                window.setTimeout(updateHistoryRecord, 10000);
            }
            if (timeInterval != 0) {
    ...
    function loadAwardTimes() {
        $.get('/xyft/ajax', { ajaxhandler: 'GetXyftPlanAwardTimes', t: Math.random() }, function (data) {
            //请求到数据后需要做的事情
...
function updateHistoryRecord() {
    $.get('/xyft/numberplan', { t: Math.random() }, function (text) {
        $('#planList').html(text);
    });
}
