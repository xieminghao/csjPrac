...
$.ajax({
	...
	success: function(res){
        ...
        $('.btn_share_html').click(function () {
            $('#copyInput').val('<iframe src="http://www.1395p.com/gdkl10/shipin/?mode=iframe" width="980" height="630" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>');
            $('#copyInput').select();
            try {
        ...
        $('.btn_share_url').click(function () {
            $('#copyInput').val('http://www.1395p.com/gdkl10/shipin/');
            $('#copyInput').select();
            try {
                window.clipboardData.setData('Text', $('#copyInput').val());
                alert("复制成功");
