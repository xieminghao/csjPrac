...
function awardNewData() {
    $.get("/xyft/ajax", { ajaxHandler: 'GetNewestRecord', t: Math.random() }, function (data) {
        ...
        for (var i = 0; i < numbers.length; i++) {
            html += '<i class="lot-no' + numbers[i] + '" name="num-' + numbers[i] + '"></i>';
        }
        html += '</td>';
        html += '<td name="spj"></td>';
        //html += '<td name="ch"></td>';
...
function reloadData() {
    ...
    $.get("/xyft/ballrulestat", { ball: ball, count: count, t: Math.random() }, function (data) {
        var chartDatas = [];
        if (data.success) {
            for (var i = 1; i < 11; i++) {
