...
function tick() {
    ...
    $.get('/shishicai/getcurrentbetgame', { t: Math.random() },
        function (result) {
            result = eval('(' + result + ')');
...
function appendTable(betGamesItem, starStr) {
    ...
    for (var i = 0; i < betData.length; i++) {
        ...
        for (var j = 0; j < betData[i].data.length; j++) {
            ...
            newTable = newTable + "<td class='betgame-td3'>";
        ...
        for (var k = tdCount; k < 3; k++) {
            newTable = newTable + "<td class='betgame-td3'>&nbsp;</td>";
        }
...
$.ajax({
    ...
    success: function(res){
        ...
        $(function () {
            ...
            $.get('/shishicai/gethistoryandbetgame', { count: 20, t: Math.random() },
                function (result) {
                    result = eval('(' + result + ')');
                    var htmlrs = "";
                    ...
                    for (var j = 0; j < result.itemArray.length; j++) {
                        ...
                        if (htmlrs) {
                            table += '<tr><td style="border-bottom:none;background-color: #F0F0F0;">';
                            table += htmlrs.head;
